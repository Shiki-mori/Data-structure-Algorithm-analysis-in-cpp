...
## 1.6.5 类模板的分离式编译
像正规的类一样，类模板可以完全在声明中实现，也可以把接口从实现中分离出去。然而，模板分离式编译的编译器支持始终不足。因此很多情形下，整个类模板及其实现一起被放到单一的头文件中。标准库的流行实现方法都遵循这种策略来实现类模板。  

附录A描述模板分离编译中涉及的机制。  
模板接口的声明是我们期望的：成员函数不提供实现。但是成员函数的实现会导致复杂的语法，特别是operator=这类函数。当编译时，编译器常常报出丢失函数的错误，而避免这个问题需要特殊平台。  

因此，本书在线代码中，实现所有类模板完全都是在单一头文件的声明中进行的。这似乎是避免跨平台编译问题的唯一办法。本书中提供的类接口就像分离编译是依序进行，因为这易于展现。在特殊平台可以机械地将单独头文件实现转换成分离式编译实现。  

...
```
erase操作对list是O(1)，对vector是O(N)。
## 3.3.3 const_iterators
*itr的结果不只是迭代器正在指向的项的值，还有该项本身。  
把一个集合中的所有项改写成一个特定值。  
```cpp
template <typename Container, typename Object>
void change(Container &c, const Object &newValue) {
  // Container::iterator itr = c.begin();  错误用法
  typename Container::iterator itr = c.begin();  //正确用法
  // auto itr = c.begin();  正确用法
  while (itr != c.end()) {
    *itr++ = newValue;
  }
}
```
这段代码中c以引用方式传递。  
假设c以常量引用方式const Container &c传递，这说明不想对c做出改变，编译器将禁止调用c的修改函数，*itr = 0; 这类语句将是非法的。  
对于这种情况，STL对每个集合容器提供const_iterator。与iterator的主要区别在于，const_iterator的operator*返回一个常量引用，因此\*itr不能作为左值出现。  

编译器要求使用const_iterator来遍历常量集合。提供两种版本的begin和end完成遍历：  
- iterator begin()  
- const_iterator begin() const  
- iterator end()  
- const_iterator end() const  
对常量容器调用begin或end，将返回const_iterator，该返回值不可赋值给iterator。  
auto可以推算出使用iterator还是const_iterator。  

C++11提供非Container成员函数的自由函数begin和end，调用方法为begin(c), end(c)。  
```cpp
// 3.6打印任意容器
